...
# answer: rs1861: 0.9430874,  rs3813199: 0.8456588,  rs3128342: 0.6948789, rs11804831: 0.9460141

(ii) What are the minor allele frequencies (MAFs) for these four SNPs?
        
# answer is 1 - major allele freq. 0.0569126, 0.1543412, 0.3051211, 0.0539859
...
#hwe[hwe$ID%in%c('rs1861', 'rs3813199', 'rs3128342', 'rs11804831'),] 
...
# Your code here...rs1861(0.274719) , rs3813199(P=1.000000), rs3128342(0.330273), and rs11804831(0.113354)
...
```{r}
# Load genotype data
geno_subset <- fread("./02_activities/data/geno.additive.A1.raw")
...
model<- lm (PHENOTYPE ~ rs1861_C, data = geno_subset, family = normal)
summary(model)
```
...
# Your answer here...beta coefficient is 0.96698, a positive number, indicating that each additional copy of the C allele is associated with increased odds of being a case. Positive increase of disease risk.


(iii) Plot the scatterplot of phenotype versus the genotype of SNP rs1861. Add the regression line to the plot.

```{r,message=FALSE, warning=FALSE}
# Make prediction over continuous range (for smooth line)
geno_range <- data.frame(
  rs1861_C = seq(0, 2, length.out = 100)
)

# Predict fitted values from linear regression model
geno_range$predicted_y <- predict(model, newdata = geno_range)

# Observed data
plot_data <- geno_subset %>%
...
print(p1)
ggsave("./02_activities/data/p1_A1_plot.png", plot = p1, width = 7, height = 5, dpi = 300)
...
```{r}
geno_subset_res <- geno_subset  # make a copy
geno_subset_res[, 7:ncol(geno_subset_res)] <- as.data.frame(
...
model<- lm (PHENOTYPE ~ rs1861_C, data = geno_subset_res, family = normal)
summary(model)
```
...
print(p3)
ggsave("./02_activities/data/p3_A1_plot.png", plot = p3, width = 7, height = 5, dpi = 300)
...
The Additive model fits better, because it has higher F value (371.7) compared to the recessive model (358.6). I learnt it from Google.
